        ...
        if useMockData {
            mockResponse { request in
                ...
                for match in matches {
                    ...
                    let value = String(query[query.index(query.startIndex, offsetBy: match.range(at: 2).location) ..< query.index(query.startIndex, offsetBy: match.range(at: 2).location + match.range(at: 2).length)])
                    switch key {
                    case "skip":
                        skip = Int(value)
                    case "limit":
